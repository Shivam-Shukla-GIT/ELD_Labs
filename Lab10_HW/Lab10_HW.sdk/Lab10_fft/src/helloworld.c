...
void Log_output(float complex FFT_input[N], float complex FFT_output[N]){
	for (int i=0; i < N; i++){
		FFT_output[i] = clogf(crealf(FFT_input[i])) + clogf(cimag(FFT_input[i]));
	}
}
